...
print("Checkpoint 0")
Directory_path = '/n/holylfs05/LABS/hernquist_lab/AGN_Feedback_Fire/t95_3000_tor4_lower/output/'
...
for fname in List_diir:
    ...
    field      = temperature*Gas_mass
    field_mass = Gas_mass
    Particles  = sphviewer.Particles(Gas_location, field, hh)
    Particles_m= sphviewer.Particles(Gas_location, field_mass, hh)

    Scene   = sphviewer.Scene(Particles)
    Scene_m = sphviewer.Scene(Particles_m)
  
    fig = plt.figure(1,figsize=(15,5))
    fig.suptitle(r"Hot thermal jet with lower energy flux : Snapshot #" + str(sorted[oo]+1) + " with Temperature & Mask 5", fontsize=17, x=0.5, y=1.5) #set a figure title on top
    ...
    Particles1  = sphviewer.Particles(Gas_location[mask_z], field[mask_z], hh[mask_z])
    Particles1_m= sphviewer.Particles(Gas_location[mask_z], field_mass[mask_z], hh[mask_z])
    ...
    Render1_m.set_logscale()

    img1    = Render1.get_image()
    img1_m  = Render1_m.get_image()
    extent1 = Render1.get_extent()
    divider = make_axes_locatable(ax1)
  
    image1 = ax1.imshow(img1-img1_m, extent=extent1, origin='lower', cmap=plt.cm.jet, vmin=5.5, vmax=7, rasterized=True)
    ...
    Particles2  = sphviewer.Particles(Gas_location[mask_x], field[mask_x], hh[mask_x])
    Particles2_m= sphviewer.Particles(Gas_location[mask_x], field_mass[mask_x], hh[mask_x])
    ...
    Render2_m.set_logscale()

    img2    = Render2.get_image()
    img2_m  = Render2_m.get_image()
    extent2 = Render2.get_extent()
  
    image2 = ax2.imshow(img2-img2_m, extent=extent2, origin='lower', cmap=plt.cm.jet, vmin=5.5, vmax=7, rasterized=True)
    ...
    Particles3  = sphviewer.Particles(Gas_location[mask_y], field[mask_y], hh[mask_y])
    Particles3_m= sphviewer.Particles(Gas_location[mask_y], field_mass[mask_y], hh[mask_y])
    ...
    Render3_m.set_logscale()

    img3    = Render3.get_image()
    img3_m  = Render3_m.get_image()
    extent3 = Render3.get_extent()
  
    image3 = ax3.imshow(img3-img3_m, extent=extent3, origin='lower', cmap=plt.cm.jet, vmin=5.5, vmax=7, rasterized=True)
    ...
    Default_dir = '/n/home13/nqutob/AGN_Feedback/ion_snapshots/' + 't95_3000_tor4_lower_mask5_temp'
    #Default_dir_pdf = '/n/home13/nqutob/AGN_Feedback/ion_snapshots/' + 'm12_mcvt_m2_t95_3000_tor4_lower_mask_pdf'
    ...
    plt.savefig(Default_dir + '/Fire' + str(sorted[oo]).zfill(3) + '_t95_3000_tor4_lower_mask5_temp' + '.png', dpi = 600, transparent = True, bbox_inches='tight')
    #plt.savefig(Default_dir_pdf + '/Fire' + str(sorted[oo]).zfill(3) + '_m12_mcvt_m2_t95_3000_tor4_lower_mask_O6' + '.pdf', dpi = 600, transparent = True, bbox_inches='tight')
