...
for fname in List_diir:
    ...
    Gas_mass = 1e10*data["PartType0"]["Masses"][:] # IMPORTANT FOR LATER ANALYSIS! 
    ...
    Particles = sphviewer.Particles(Gas_location, Magnesium2_mass, hh) # CHANGE PARAMETER HERE!
    Scene = sphviewer.Scene(Particles)
    
    fig = plt.figure(1,figsize=(15,5))
    fig.suptitle(r"Hot thermal jet but with lower energy flux: Snapshot #" + str(sorted[oo]+1) + " with MgII Mass & Mask 200", fontsize=17, x=0.5, y=1.5) #set a figure title on top
    ...
    Particles1 = sphviewer.Particles(Gas_location[mask_z], Magnesium2_mass[mask_z], hh[mask_z]) # CHANGE PARAMETER HERE! 
    ...
    Particles2 = sphviewer.Particles(Gas_location[mask_x], Magnesium2_mass[mask_x], hh[mask_x]) # CHANGE PARAMETER HERE! 
    ...
    Particles3 = sphviewer.Particles(Gas_location[mask_y], Magnesium2_mass[mask_y], hh[mask_y]) # CHANGE PARAMETER HERE! 
    ...
    Default_dir = '/n/home13/nqutob/AGN_Feedback/ion_snapshots/' + 'Mg2_t95_3000_tor4_lower_mask200'
    ...
    plt.savefig(Default_dir + '/Fire' + str(sorted[oo]).zfill(3) + '_Mg2_t95_3000_tor4_lower_mask200' + '.png', dpi = 600, transparent = True, bbox_inches='tight')
